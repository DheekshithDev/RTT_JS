...
(async () => {
  ...
  fs.readFile('./DataDirect3Tor.json', 'utf8', (err, data) => {
    if (err) {
    ...
    fs.writeFile('./DataDirect3Tor.json', updatedJsonData, (err) => {
      if (err) {
        console.error('Error writing to file', err);
      } else {
        console.log('Performance data saved to DataDirect3Tor.json');
      }
    });
